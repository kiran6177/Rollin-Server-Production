...
import { MongoAdminRepository, MongoTheatreRepository, MongoUserRepository } from '../repositories/index.js'
...
export class AuthHandler{
                ...
                if(decoded){
                    ...
                    if(req.cookies && req.cookies['adminRefreshToken']){
                        ...
                        if(adminRefreshToken){
                            ...
                            if(decoded){
                                if(decoded.role === 'ADMIN'){
                                    const isAllowed = await adminRepository.findAdminById(decoded.id)
                                    console.log(isAllowed);
                                    if(isAllowed){
                                            const adminWOP = {
                                                id:isAllowed._id,
                                            ...
                                            const newAccessToken = await createToken({...adminWOP,role:'ADMIN'});
                                ...
                                if(decoded.role === 'THEATRE'){
                                    ...
                                    if(isAllowed){
                                        if(isAllowed.isBlocked || !isAllowed.isVerified){
                                            ...
                                            const newAccessToken = await createToken({...theatreWOP,role:'THEATRE'});
    ...
    static async isPassWithin(req,res,next){
        try{
            const userRepository = new MongoUserRepository();
            if(req.headers && req.headers['authorization']){
                const access_token = req.headers['authorization'].split(' ')[1]
