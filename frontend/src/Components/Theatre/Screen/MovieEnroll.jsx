...
function MovieEnroll() {
    ...
    useEffect(()=>{
      ...
      useEffect(()=>{
        ...
        dispatch(resetMovieList())
        dispatch(theatreGetAllMovies({filters:{page:1,languages,genres,search:debouncedValue},token:theatreToken}))
        }
      },[debouncedValue])
  ...
  return (
    ...
    <Toaster richColors />
        <div className='p-12'>
            <h1 className='text-white font-semibold text-3xl mb-10'>MOVIES</h1> 
        ...
        <div className='flex flex-col lg:flex-row gap-6 mt-4'>
          ...
          <div className='border-2 border-[#f6ae2d] xl:w-[40%] bg-black rounded-sm p-6'>
            ...
            <div ref={scrollRef} className='my-6 mx-auto flex flex-wrap gap-6 justify-evenly '>
            {
                ...
                <InfiniteScroll dataLength={moviesList?.length} scrollThreshold={'350px'} scrollableTarget={scrollRef} next={nextPage} loader={<ScaleLoader className='absolute -bottom-[6rem] ' color='#f6ae2d'/>} hasMore={true} className='my-6 py-4 px-2 mx-auto flex flex-wrap gap-7 relative  justify-evenly scrollbar-none'>
                {
